...
// Only counts attempts from visible, non‑expired quizzes at or below the student's level
exports.getLeaderboard = async (req, res) => {
  try {
    ...
    const query =
      ...
      // Count only attempts that are on the student's current level (or lower)
      ...
      "JOIN question_types qt ON qa.type_id = qt.id " +
      "LEFT JOIN student_subject_level ssl ON ssl.student_id = u.id AND ssl.subject_id = qt.subject_id " +
